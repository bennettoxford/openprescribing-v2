...
@pytest.mark.django_db(databases=["data"])
def test_get_org_date_ratio_matrix_prescribing_vs_list_size_multiple_queries(
    rxdb, sample_data
):
    analysis = Analysis(
        queries=(
            ...
            AnalysisQuery(
                ntr_query=BNFQuery(bnf_codes=["1001030U0AAACAC"]),
                dtr_query=ListSizeQuery(),
            ),
        ),
        org_id="ICB01",
    ...
    # TODO: upgrade alternative implementation
    expected_odm = get_org_date_ratio_matrix_alternative(sample_data, analysis)
